...
class TestAnonymousDataLeakagePrevention:
    ...
    def test_short_notice_flag_hidden_from_anonymous_users(self, client, test_member, app):
        ...
        # Verify slot shows as 'reserved' not 'short_notice' (sparse format)
        court_data = data['courts'][0]  # First court
...
class TestAnonymousErrorHandling:
    ...
    def test_invalid_date_error_handling(self, client):
        ...
        for invalid_date in invalid_dates:
            response = client.get(f'/api/courts/availability?date={invalid_date}')
    ...
    def test_system_error_handling_for_anonymous_users(self, client, app):
        ...
        response = client.get('/api/courts/availability?date=9999-12-31')
    ...
    def test_rate_limit_error_format_for_anonymous_users(self, client):
        ...
        with client.application.test_request_context('/api/courts/availability'):
...
class TestPerformanceValidation:
    ...
    def test_data_filtering_performance_impact(self, client, test_member, app):
        ...
        # Allow up to 500% overhead for filtering (generous threshold for test environment)
        # Note: In test environments, small timing variations can cause flaky results
        assert anonymous_time <= authenticated_time * 5.0
    ...
    def test_memory_usage_with_large_dataset(self, client, test_member, app):
        ...
        # Verify all slots are properly filtered (sparse format - only occupied slots returned)
        for court_data in data['courts']:
            for slot in court_data['occupied']:
                if slot['status'] == 'reserved':
                    assert slot['details'] is None  # Should be filtered out for anonymous
    ...
    def test_response_size_comparison(self, client, test_member, app):
        ...
        anonymous_response = client.get('/api/courts/availability?date=2026-12-05')
        ...
        with client:
            ...
            authenticated_response = client.get('/api/courts/availability?date=2026-12-05')
